...
f_model = function(
  ...
  f_par = function(par, cats, .t, .p, .l, nruns, Num_months, baseline_scen, control_scen, ill_month){    
    out = foreach(seed = 1:nruns) %dopar% {
      ...
      for(i in 1 : Num_months){        
        ...
        if(is.null(baseline_scen)) p = par else if(i %in% ill_month) p = par[[2]] else p = par[[1]] 
        
        pop[[i+1]] <- pmap(
          list(pre = pre, post = post, .t = .t, .p = .p) %>% c(., .l, p), 
          function(pre, post, .t, .p, .n, .j, .a, .f, .m, 
                   N0, g, pt, pf, pfm, ld, ady, mvl, o, d, c, lw, ccy, fv, hr, hrm, 
                   hv, m, mv, dm, plp, pfp, fc, dif, lsr, lnh, he, ir, iph
          ){
            set.seed(seed)
            ...
            Df=df %>% mutate(
              sum = rowSums(.), come = B+Gi, leave = sum-come, diff =come-leave, new = post+diff, 
              NOftk = O*(.j+.a) + C*(.a*.m), 
              PopGro = diff, 
      ...
      subtot = pmap(tags, 
                    function(p, o, n, j, a, f, m){
                      ...
                      rs = rowSums(df) 
                      mp = map(.l, function(x) apply(df, 1, function(y) (y*unlist(x)) %>% sum)) %>% data.frame
  ...
  if(is.null(baseline_scen)){
    output = map(par, f_par, cats, .t, .p, .l, nruns, Num_months, baseline_scen, control_scen, ill_month) 
    names(output) = scen
  } else {
    output = f_par(par, cats, .t, .p, .l, nruns, Num_months, baseline_scen, control_scen, ill_month)
    output = list(output)
    names(output) = scen[[1]]
  }
  output
