ahle_sr_offtake <- function(
    file, sheet = 1, par_names = "AHLE Parameter", ignore = "Notes", 
    nruns = 1000, Num_months = 12, 
    baseline_scen = NULL, control_scen = NULL, ill_month = NULL
){
  if (!require("pacman")) install.packages("pacman")
  p_load("tidyverse", "readxl", "freedom", "truncnorm","data.table", "doParallel", "abind")
        ...
        map(. , 
            function(x, name){
              ...
              par = list(
                ...
                mv = Man_value, 
                dm = list(DM_req_prpn_NF, DM_req_prpn_NM, DM_req_prpn_JF, 
                          DM_req_prpn_JM, DM_req_prpn_AF, DM_req_prpn_AM), 
  ...
  f_par = function(
    ...
    out = foreach(seed = 1:nruns) %dopar% {
      ...
      subtot = pmap(tags, 
                    function(p, o, n, j, a, f, m){
                      ...
                      rs = rowSums(df) 
                      mp = map(.l, function(x){
                        apply(df, 1, function(y) (unlist(x) * y) %>% sum)
                      } ) %>% data.frame
    ...
    ou %>% map(., ~ .x [ Num_months, ] ) %>% do.call(rbind, .) %>%
      ...
      mutate_all(
        function(x){
          lab=c("B", "Gi", "Go", "D", "O", "C", 
                "sum", "come", "leave", "diff", "new", 
                "NOftk", 
                "Num_culled",# JUL2023
                "PopGro", "TotNChg", "TotMort", 
                "QLwKg", "OftkLw", "QMeat", 
                "QManu", "QHides", "QMilk", "QWool", 
                "CumDM", "ValOftk", "ValHerd", 
                "TotVal", 
                "ValManu", "ValHides", "ValMilk", "ProdVal", 
                "FdCost", "LbCost", "HthCost", "CapCost", 
                "IstCost", "TotExp", "GrsMrg", 
                "NF", "NM", "JF", "JM", "AF", "AM", 
                "Overall", "NCom", "JCom", "ACom", "FCom", "MCom")
          name=c("Births", "Growth in", "Growth out", "Deaths", "Offtakes", "Culls", 
                 "Sum", "Inflow", "Outflow", "Difference", "Population", 
                 "Num Offtake", 
                 "Number culled",# JUL2023
                 "Cml Pop Growth", "Total Number Increase", "Total Mortality", 
                 "Population Liveweight (kg)", "Offtake Liveweight (kg)", "Meat (kg)", 
                 "Manure", "Hides", "Milk", "Wool", 
                 "Cml Dry Matter", "Value of Offtake", "Value of Herd Increase",
                 "Value of Herd Increase plus Offtake", 
                 "Value of Manure", "Value of Hides", "Value of Milk", "Total Production Value", 
                 "Feed Cost", "Labour Cost", "Health Cost", "Capital Cost",
                 "Infrastructure Cost", "Total Expenditure", "Gross Margin", 
                 "Neonatal Female", "Neonatal Male", 
                 "Juvenile Female", "Juvenile Male", 
                 "Adult Female", "Adult Male", 
                 "Overall", "Neonatal Combined", "Juvenile Combined", 
                 "Adult Combined", "Female Combined", "Male Combined")
          for(i in 1:length(lab)) x[x == lab[[i]]] <- name[[i]]; x
        }
      ) %>% .[.$Item != "",]    
  }
  output =map(b_scen, f_par, c_scen[[1]], age, sex, cats, .g, .t, .p, .l, data, 
              file, sheet, par_names, ignore, nruns, 
              Num_months, baseline_scen, control_scen, ill_month)
...
output = ahle_sr_offtake(
  "Copy of AHLE scenario parameters-20221202 AFTER GEMMA SR UPDATE corrected for less scenario(32016).xlsx",
  nruns = 10, 
